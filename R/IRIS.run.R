...
cat(paste0("### IRIS starts to run the domain detection! ...\n"))
###### Initialize IRIS
...
for(islice in 1:length(ResList$VList)){
  ...
  colnames(ResList$VList[[islice]]) <- colnames(B)
}
VList = ResList$VList
rm(ResList)
...
ResList= IRIS_ref_iter(countList,B,AList,VList,Mu_STR$STRList,Mu_STR$Mu)
obj = ResList$Obj 
...
cat(paste0("## IRIS Finished the domain detection! ...\n"))
...
colnames(spatialDomain)[colnames(spatialDomain) == "kmeans_cluster"] <- "IRIS_domain"
